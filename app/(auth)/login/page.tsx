  ...
  try {
    ...
    if (user) {
      if (password === user.username) {
        ...
        return { success: false, message: "Incorrect password (use username as password)" };
      }
    } else {
      return { success: false, message: "User not found" };
    }
...
export default function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div className="max-w-md w-full bg-white dark:bg-gray-700 p-8 rounded-lg shadow-md mt-10">
        <form onSubmit={handleLogin} className={cn("flex flex-col gap-6 max-w-md w-full mx-auto")}>
          <div className="text-center space-y-1">
            <h1 className="text-2xl font-bold">Login to your account</h1>
            <p className="text-sm text-muted-foreground">
              Enter your email and your username as password to login
            </p>
          </div>
